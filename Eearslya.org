...
#+BEGIN_SRC emacs-lisp
  (use-package pipenv
    ...
    :init
    ;; Define a custom projectile handler, as pipenv.el is
    ;; apparently bad at finding the project root.
    (defun pipenv-projectile-after-switch-custom ()
      (let ((default-directory (projectile-project-root)))
        (pipenv-deactivate)
        (when (pipenv-project?)
          (setq python-shell-virtualenv-root (pipenv-venv))
          (pipenv-activate))))
    (setq pipenv-projectile-after-switch-function #'pipenv-projectile-after-switch-custom))
#+END_SRC
